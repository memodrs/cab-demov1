...
public class Verstaerkung extends CardStateSpell {
	...
	public boolean isEffektPossible(CardGame cardGame) {
		return  cardGame.getOwnerOfCard(this).hasBoardPlace() && cardGame.getOwnerOfCard(this).handCards.stream().anyMatch(card -> Art.Mensch.equals(card.art));
    }

	public List<CardState> getCardListToSelect(CardGame cardGame) {
		return cardGame.getOwnerOfCard(this).handCards.stream().filter(card -> card.art == Art.Mensch).collect(Collectors.toList());
    }
}